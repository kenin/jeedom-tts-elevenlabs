...
<form class="form-horizontal adm_form">
	<fieldset>
		...
		<div class="form-group">
			...
			<div class="col-sm-6">
				<select class="configKey form-control" id="model_adm_select" data-l1key="model" disabled>
					<?php
		...
		<div class="form-group conf-lang-group" style="display:none">
			<label class="col-sm-4 control-label">{{Langue}}</label>
			<div class="col-sm-6">
				<select class="configKey form-control" id="language_adm_select" data-l1key="language" >
					<?php
					foreach ($models as $model) {
...
<script>
	setTimeout(() => {
		...
		if (apiKey != null && apiKey != "") {
			document.querySelectorAll('.adm_form .configKey[data-l1key=voice]').forEach((elem) => {
				elem.disabled = false;
			});
			document.querySelector('.adm_form .configKey[data-l1key=cacheLifetime]').disabled = false;
			document.querySelector('.adm_form .configKey[data-l1key=model]').disabled = false;
			//si model = eleven_turbo_v2_5 on affiche la langue
			if(document.querySelector('.adm_form .configKey[data-l1key=model]') != null && document.querySelector('.adm_form .configKey[data-l1key=model]').value == "eleven_turbo_v2_5")
			{
				document.querySelector(".conf-lang-group").style.display = "block";
			}
		}
		...
		document.querySelector("#btn_adm_test").addEventListener("click", function(e) {
			...
			let language = document.querySelector('.adm_form .configKey[data-l1key=language]').value;
		...
		document.querySelector(".adm_form .configKey[data-l1key=model]").addEventListener("change", function(e) {
			...
			{
				document.querySelector(".conf-lang-group").style.display = "none";
			}
		}, true);
